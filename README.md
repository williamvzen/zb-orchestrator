...
| Item | Role |
|------|------|
| [`zb-agent.py`](zb-agent.py) | Interactive navigator over `~/zb-projects`; opens one project or a multi-root workspace in **Cursor Agent** (or IDE only), or **`--cli-only`** for a shell in the repo. **`--cleanup-zb-agent-docker`** runs `scripts/cleanup-zb-agent-docker.sh` (stops all **running** Docker containers; no Cursor). Optional `--ticket`, `--reason` (OpenAI), `--noops`. |
...
| [`cleanup-zb-agent-docker-agent`](cleanup-zb-agent-docker-agent) | Opens this repo in Cursor with a seed for **cleanup-zb-agent-docker** (Agent workflow only; for direct cleanup use **`zb-agent --cleanup-zb-agent-docker`**). |
...
| [`.cursor/skills/cleanup-zb-agent-docker/SKILL.md`](.cursor/skills/cleanup-zb-agent-docker/SKILL.md) | Stop/remove all running Docker containers via `scripts/cleanup-zb-agent-docker.sh`. |
